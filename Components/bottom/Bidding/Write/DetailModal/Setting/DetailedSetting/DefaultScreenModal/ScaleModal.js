...
class ScaleModal extends Component {
  ...
  render() {
    return (
        ...
        onRequestClose={() => {
        this.props.scaleModalVisible(this.state.size);
    }}
    backdrop={true}
    >
      <View style={{flex:1}}>
        <Header style ={{justifyContent:'space-between'}}>
          <Icon name='ios-arrow-back' onPress={()=>{this.props.scaleModalVisible(this.state.size);}}/>
          <Text>평 수</Text>
          <Text/>
        </Header>
        ...
        <View style={{alignItems:'center', justifyContent:'center', width:'100%'}}>
          <TouchableOpacity 
            onPress={()=>this.props.scaleModalVisible(this.state.size)}
